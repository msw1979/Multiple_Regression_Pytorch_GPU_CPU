# Multiple Regression Using Pytorch (GPU & CPU)
This is an example of Multiple Regression using Pytorch (GPU and CPU). The data used here is concrete data and can be downloaded from: https://s3-api.us-geo.objectstorage.softlayer.net/cf-courses-data/CognitiveClass/DL0101EN/labs/data/concrete_data.csv. The code split the data to training and validation data sets and normalize the feature part, then load it to the model for training and validation. The model is created using subclass and sequential and user can use either approach. The code will generate loss and accuracy data and plot them. It also plots the validation data with prediction.
The figure below show the loss and accuracy versus epoch:
